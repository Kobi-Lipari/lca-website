...
export const onRequestGet: PagesFunction<Env> = async (context) => {
  ...
  // Check if user is privileged (to include non-visible LCA tournaments)
  ...
  // 1. Fetch LCA tournaments from tournaments table
  //    Map them to the clearinghouse shape
  ...
  const chQuery = `SELECT *, 0 AS is_rated, NULL AS registration_status, NULL AS entry_fee, NULL AS sections, NULL AS rounds, NULL AS status, NULL AS club_id, NULL AS club_color, NULL AS club_name FROM clearinghouse WHERE ${chConditions.join(' AND ')} ORDER BY start_date ASC`
  ...
  const lcaTournaments = (lcaRows.results ?? []).map(t => {
    ...
    // Apply state/upcoming filters to LCA rows too
  ...
  // 5. Merge and sort by start_date
