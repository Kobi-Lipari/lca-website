...
const heroSlides = [
  { label: 'Louisiana Chess Community', src: null },
  { label: 'Club tournaments across the state', src: null },
  { label: 'Scholastic chess in Louisiana', src: null },
]
...
function HeroSlideshow() {
  ...
  const gradients = [
    'linear-gradient(135deg, #1a2744 0%, #2d4a8c 100%)',
    'linear-gradient(135deg, #1a3a2a 0%, #1a2744 100%)',
    'linear-gradient(135deg, #2a1a10 0%, #1a2744 100%)',
  ]
    ...
    <div className="relative overflow-hidden border-b border-border" style={{ height: 420 }}>
      <div className="flex h-full transition-transform duration-500 ease-in-out" style={{ transform: `translateX(-${current * 100}%)` }}>
        {heroSlides.map((slide, i) => (
          <div key={i} className="relative flex h-full min-w-full items-center justify-center" style={{ background: gradients[i] }}>
            <div className="absolute inset-0 flex items-center justify-center opacity-10">
              <Trophy className="size-32 text-white" strokeWidth={0.5} />
            </div>
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-end pb-14 px-6 text-center text-white">
        ...
        <div className="mt-5 flex gap-6">
          {[{ n: '340+', l: 'members' }, { n: '8', l: 'clubs' }, { n: '50+', l: 'events/year' }].map((s) => (
...
export function HomePage() {
  ...
  return (
    <div>
        ...
        <div className="flex flex-col border-b border-border sm:border-b-0 sm:border-r">
          <div className="flex items-center justify-between border-b border-border bg-muted/30 px-4 py-2">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs font-semibold text-foreground">Tournaments</span>
            </div>
            <Link to="/tournaments" className="flex items-center gap-0.5 text-[10px] text-[#c8a94a] hover:underline">
          ...
          <div className="overflow-y-auto" style={{ maxHeight: 240 }}>
            {loadingTournaments ? (
              ...
              tournaments.map((t) => {
                ...
                return (
                  <Link key={t.id} to={`/tournaments/${t.id}`} className="flex items-center justify-between border-b border-border px-4 py-2.5 transition-colors last:border-b-0 hover:bg-muted/30" style={{ backgroundColor: clubColorTint(color, 0.05) }}>
                    <div className="min-w-0">
                      <p className="truncate text-xs font-medium text-foreground">{t.name}</p>
                      <p className="text-[10px] text-muted-foreground">{t.date} · {t.location}</p>
                    </div>
        ...
        <div className="flex flex-col border-b border-border sm:border-b-0 sm:border-r">
          <div className="flex items-center justify-between border-b border-border bg-muted/30 px-4 py-2">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs font-semibold text-foreground">Latest from Facebook</span>
            </div>
            <a href="https://www.facebook.com/LouisianaChessAssociation" target="_blank" rel="noopener noreferrer" className="flex items-center gap-0.5 text-[10px] text-[#1877F2] hover:underline">
              Follow <ArrowRight className="size-3" />
            </a>
          </div>
          <div className="overflow-hidden" style={{ maxHeight: 240 }}>
            <div className="fb-page" data-href="https://www.facebook.com/LouisianaChessAssociation" data-tabs="timeline" data-width="500" data-height="240" data-small-header="true" data-adapt-container-width="true" data-hide-cover="true" data-show-facepile="false" />
          </div>
        </div>
        ...
        <div className="flex flex-col">
          <div className="flex items-center justify-between border-b border-border bg-muted/30 px-4 py-2">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs font-semibold text-foreground">Clubs</span>
            </div>
            <Link to="/clubs" className="flex items-center gap-0.5 text-[10px] text-[#c8a94a] hover:underline">
          ...
          <div className="overflow-y-auto" style={{ maxHeight: 240 }}>
            {loadingClubs ? (
              ...
              clubs.map((club) => {
                ...
                return (
                  <Link key={club.id} to={`/clubs/${club.id}`} className="flex items-center gap-3 border-b border-border px-4 py-2.5 transition-colors last:border-b-0 hover:bg-muted/30" style={{ backgroundColor: clubColorTint(color, 0.05) }}>
                    <span className="h-2 w-2 shrink-0 rounded-full" style={clubAccentStyle(color)} />
                    <div className="min-w-0">
                      <p className="truncate text-xs font-medium text-foreground">{club.name}</p>
                      <p className="text-[10px] text-muted-foreground">{club.city}, LA{club.meeting_schedule ? ` · ${club.meeting_schedule}` : ''}</p>
                    </div>
                  </Link>
                )
              })
      ...
      <div className="bg-[#1a2744] px-6 py-12 text-center text-white">
