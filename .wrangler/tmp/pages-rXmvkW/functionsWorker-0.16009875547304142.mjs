...
// ../.wrangler/tmp/bundle-iWgJii/checked-fetch.js
...
var init_tslib_es6 = __esm({
  "../node_modules/tslib/tslib.es6.mjs"() {
    init_functionsRoutes_0_049655092631503805();
    init_checked_fetch();
...
async function updateMemberRole(db, env, memberId, role) {
  ...
  await db.prepare("UPDATE members SET role = ? WHERE id = ?").bind(role, memberId).run();
...
async function updateMemberClub(db, env, memberId, clubId) {
  const existing = await getMemberById(db, memberId);
  if (!existing) return null;
  await db.prepare("UPDATE members SET club_id = ? WHERE id = ?").bind(clubId, memberId).run();
  const updated = await getMemberById(db, memberId);
  if (updated) {
    await syncSupabaseUserMetadata(env, memberId, {
      role: updated.role,
...
async function updateMemberProfile(db, id, data) {
  ...
  await db.prepare(
    ...
    data.uscfId !== void 0 ? data.uscfId : existing.uscf_id,
    id
  ).run();
...
async function canManageTournament(db, member, tournamentId) {
  ...
  if (member.role === "club_rep" && clubId && member.club_id === clubId) {
    return true;
  }
...
async function requireRole(request, env, roles) {
  ...
  if (!allowed.includes(authed.member.role)) {
...
async function requireClubRep(request, env, clubId) {
  ...
  if (authed.member.role === "club_rep" && authed.member.club_id === clubId) {
    return authed;
  }
  return errorResponse("Forbidden", 403);
}
async function requireTournamentManager(request, env, tournamentId) {
  const authed = await requireAuthedMember(request, env);
  if (authed instanceof Response) return authed;
  const { canManageTournament: canManageTournament2 } = await Promise.resolve().then(() => (init_permissions(), permissions_exports));
  const allowed = await canManageTournament2(
    env.DB,
    authed.member,
    tournamentId
  );
...
function buildScoreMap(playerIds, games) {
  ...
  for (const id of playerIds) {
    scores.set(id, 0);
    opponents.set(id, /* @__PURE__ */ new Set());
  }
...
function computeProgressive(playerId, games) {
  ...
  const playerGames = games.filter(
    (g) => g.result !== "pending" && (g.whiteId === playerId || g.blackId === playerId)
  );
...
function computeDirectEncounter(playerId, tiedPlayerIds, games) {
  ...
  for (const game of games) {
    ...
    const inTie = tiedPlayerIds.includes(game.whiteId) && tiedPlayerIds.includes(game.blackId);
    if (!inTie) continue;
...
function getColorPreference(player) {
  ...
  if (player.colorHistory.length >= 2) {
    ...
    if (lastTwo[0] === lastTwo[1]) {
      const required = lastTwo[0] === "white" ? "black" : "white";
...
function colorViolationCost(player, color) {
  ...
  if (pref.preferred !== color) {
    return pref.strength === "strong" ? 1e4 : 1e3;
  }
  return 0;
}
function assignColors(a, b) {
  ...
  const costAWhite = colorViolationCost(a, "white") + colorViolationCost(b, "black");
  const costBWhite = colorViolationCost(b, "white") + colorViolationCost(a, "black");
  if (costAWhite !== costBWhite) {
    return costAWhite < costBWhite ? { white: a, black: b } : { white: b, black: a };
  }
  if (a.rank !== b.rank) {
  ...
  "api/admin/tournaments/[id]/pairings/generate.ts"() {
    ...
    onRequestPost = /* @__PURE__ */ __name(async (context) => {
     ...
     WHERE tournament_id = ? AND round = ? AND section = ? LIMIT 1`
      ).bind(tournamentId, body.round, body.section).first();
      if (existingRound) {
      ...
      if (body.round > 1) {
       ...
       WHERE tournament_id = ? AND round = ? AND section = ? AND result = 'pending' LIMIT 1`
        ).bind(tournamentId, priorRound, body.section).first();
        if (pendingPrior) {
          return errorResponse(
     ...
     ORDER BY COALESCE(m.uscf_rating, 0) DESC`
      ).bind(tournamentId, body.section).all();
      const players = (roster.results ?? []).map((row) => ({
        id: row.member_id,
     ...
     WHERE tournament_id = ? AND section = ? AND round < ?`
      ).bind(tournamentId, body.section, body.round).all();
      const pastGames = (priorGames.results ?? []).map((game) => ({
...
var onRequestOptions3, onRequestPatch2;
var init_club = __esm({
  "api/admin/members/[id]/club.ts"() {
    init_functionsRoutes_0_049655092631503805();
    init_checked_fetch();
    ...
    onRequestPatch2 = /* @__PURE__ */ __name(async (context) => {
      ...
      const body = await parseJsonBody(context.request);
      if (body === null || body.clubId === void 0) {
...
var onRequestOptions4, onRequestPatch3;
var init_role = __esm({
  "api/admin/members/[id]/role.ts"() {
    init_functionsRoutes_0_049655092631503805();
    init_checked_fetch();
...
var onRequestOptions5, onRequestPost2;
var init_directors = __esm({
  "api/admin/tournaments/[id]/directors.ts"() {
    ...
    onRequestPost2 = /* @__PURE__ */ __name(async (context) => {
      ...
      if (authed.member.role === "club_rep" && authed.member.club_id !== tournament.club_id) {
        return errorResponse("Forbidden", 403);
      }
      const body = await parseJsonBody(context.request);
      if (!body?.memberId) {
     ...
     VALUES (?, ?)`
      ).bind(tournamentId, body.memberId).run();
...
var onRequestOptions6, onRequestPost3;
  ...
  "api/admin/tournaments/[id]/games.ts"() {
    ...
    onRequestPost3 = /* @__PURE__ */ __name(async (context) => {
      ...
      ON CONFLICT(tournament_id, round, board, section) DO UPDATE SET
        ...
        ).bind(tournamentId, body.round, board, body.section).first();
        if (game) created.push(game);
      }
...
var onRequestOptions7, onRequestPost4;
...
// utils/tournament-manage.ts
function computeStandings(games, roster) {
  ...
  for (const game of games) {
    if (game.result === "pending") continue;
    const white = game.white_member_id ? standings.get(game.white_member_id) : void 0;
    const black = game.black_member_id ? standings.get(game.black_member_id) : void 0;
    if (game.result === "bye") {
...
function parseTournamentSections(sectionsJson) {
  try {
...
var init_tournament_manage = __esm({
  "utils/tournament-manage.ts"() {
    init_functionsRoutes_0_049655092631503805();
    init_checked_fetch();
    __name(computeStandings, "computeStandings");
    __name(parseTournamentSections, "parseTournamentSections");
  }
});

// api/admin/tournaments/[id]/manage.ts
var onRequestOptions8, onRequestGet;
var init_manage = __esm({
  "api/admin/tournaments/[id]/manage.ts"() {
    init_functionsRoutes_0_049655092631503805();
    init_checked_fetch();
    init_auth();
    init_tournament_manage();
    init_response();
    ...
    onRequestGet = /* @__PURE__ */ __name(async (context) => {
     ...
     ORDER BY g.round ASC, g.section ASC, g.board ASC`
      ...
      const rosterRows = roster.results ?? [];
     ...
     WHERE td.tournament_id = ?`
      ...
      return jsonResponse({
        tournament: {
          ...tournament,
          sections: parseTournamentSections(tournament.sections)
        },
...
var onRequestOptions9, onRequestPatch4;
var init_id = __esm({
  "api/admin/tournaments/[id].ts"() {
    init_functionsRoutes_0_049655092631503805();
    init_checked_fetch();
    ...
    onRequestPatch4 = /* @__PURE__ */ __name(async (context) => {
     ...
     WHERE id = ?`
        ...
        body.venue !== void 0 ? body.venue : existing.venue,
        body.date ?? existing.date,
        body.endDate !== void 0 ? body.endDate : existing.end_date,
        body.entryFee ?? existing.entry_fee,
        sections,
        body.rounds ?? existing.rounds,
        body.maxPlayers !== void 0 ? body.maxPlayers : existing.max_players,
...
async function fetchUscfRatingFromWeb(uscfId) {
  ...
  const ratingMatch = html.match(/Regular\s+Rating[^0-9]*(\d{3,4})/i) ?? html.match(/Rating[^0-9]*(\d{3,4})/i);
  const nameMatch = html.match(/Name:\s*<\/[^>]+>\s*([^<\n]+)/i) ?? html.match(/<title>\s*([^<|]+)/i);
  return {
    uscfId: normalizedId,
...
var init_id2 = __esm({
  "api/tournaments/uscf/[id].ts"() {
    ...
    onRequestGet2 = /* @__PURE__ */ __name(async (context) => {
      ...
      ).bind(uscfId.trim()).first();
      if (member && member.uscf_rating != null && !isRatingCacheStale(member.uscf_rating_updated_at)) {
        return jsonResponse({
          uscfId: uscfId.trim(),
...
var onRequestOptions11, onRequestPost5;
var init_news = __esm({
  "api/clubs/[id]/news.ts"() {
    init_functionsRoutes_0_049655092631503805();
    init_checked_fetch();
    ...
    onRequestPost5 = /* @__PURE__ */ __name(async (context) => {
     ...
     VALUES (?, ?, ?, ?, ?)`
      ).bind(id, clubId, body.title, body.newsDate, body.excerpt).run();
...
var onRequestOptions12, onRequestGet3;
...
var onRequestOptions13, onRequestPost6;
...
var onRequestOptions14, onRequestPost7;
...
var onRequestOptions15, onRequestGet4;
...
var onRequestOptions16, onRequestPost8;
var init_tournaments = __esm({
  "api/admin/tournaments.ts"() {
    ...
    onRequestPost8 = /* @__PURE__ */ __name(async (context) => {
      ...
      const sections = body.sections?.length ? body.sections : [{ name: "Open", entryFee: body.entryFee }];
...
var TIER_PRICES, onRequestOptions17, onRequestPost9;
  ...
  "api/membership/checkout.ts"() {
    ...
    onRequestPost9 = /* @__PURE__ */ __name(async (context) => {
     ...
     VALUES (?, ?, ?, 'membership', ?, 'pending')`
      ).bind(paymentId, authed.member.id, amount, tier).run();
      return jsonResponse({
        paymentId,
        tier,
        amount,
...
var onRequestOptions18, onRequestPost10;
var init_confirm = __esm({
  "api/membership/confirm.ts"() {
    ...
    onRequestPost10 = /* @__PURE__ */ __name(async (context) => {
      ...
      ).bind(body.paymentId, authed.member.id).first();
      if (!payment) {
        return errorResponse("Payment not found", 404);
      }
      if (payment.status === "completed") {
        const member2 = await context.env.DB.prepare(
          "SELECT * FROM members WHERE id = ?"
        ).bind(authed.member.id).first();
        return jsonResponse({ member: member2, alreadyConfirmed: true });
      }
...
async function completePayment(db, paymentId) {
  ...
  if (payment.type === "membership") {
    await db.batch([
      ...
      db.prepare(
        `UPDATE members SET membership_status = 'active', membership_expiry = ? WHERE id = ?`
      ).bind(oneYearFromNow2(), payment.member_id)
    ]);
    return true;
  }
...
var onRequestOptions19, onRequestPost11;
...
// api/clubs/[id].ts
var onRequestOptions20, onRequestGet5, onRequestPatch5;
var init_id3 = __esm({
  "api/clubs/[id].ts"() {
    init_functionsRoutes_0_049655092631503805();
    init_checked_fetch();
...
var onRequestOptions21, onRequestGet6;
var init_id4 = __esm({
  "api/tournaments/[id].ts"() {
    init_functionsRoutes_0_049655092631503805();
    init_checked_fetch();
...
// api/clubs.ts
var onRequestOptions22, onRequestGet7;
...
var init_health = __esm({
  "api/health.ts"() {
    ...
    init_checked_fetch();
    init_response();
    onRequestGet8 = /* @__PURE__ */ __name(async () => {
...
// api/me.ts
var onRequestOptions23, onRequestGet9, onRequestPost12, onRequestPatch6;
var init_me = __esm({
  "api/me.ts"() {
    init_functionsRoutes_0_049655092631503805();
    init_checked_fetch();
    ...
    onRequestGet9 = /* @__PURE__ */ __name(async (context) => {
     ...
     ORDER BY r.registered_at DESC`
      ...
      if (directedTournamentIds.length > 0) {
        ...
        ).bind(...directedTournamentIds).all();
...
function getSectionEntryFee(sectionsJson, sectionName, defaultFee) {
  try {
    const parsed = JSON.parse(sectionsJson);
    const match2 = parsed.find((s) => s.name === sectionName);
    return match2?.entryFee ?? defaultFee;
  } catch {
    return defaultFee;
  }
}
function paymentUrl(env, type) {
  if (type === "tournament") {
    return env.STRIPE_TOURNAMENT_PAYMENT_URL ?? "https://buy.stripe.com/test_lca_tournament_placeholder";
  }
  return env.STRIPE_MEMBERSHIP_URL ?? "https://buy.stripe.com/test_lca_membership_placeholder";
}
var onRequestOptions24, onRequestPost13;
var init_registrations = __esm({
  "api/registrations.ts"() {
    ...
    onRequestPost13 = /* @__PURE__ */ __name(async (context) => {
      ...
      ).bind(body.tournamentId, authed.member.id).first();
      if (existing) {
...
var onRequestOptions25, onRequestGet10;
var init_tournaments2 = __esm({
  "api/tournaments.ts"() {
    ...
    __name(parseSections, "parseSections");
  }
});

// ../.wrangler/tmp/pages-rXmvkW/functionsRoutes-0.049655092631503805.mjs
var routes;
var init_functionsRoutes_0_049655092631503805 = __esm({
  "../.wrangler/tmp/pages-rXmvkW/functionsRoutes-0.049655092631503805.mjs"() {
    init_generate();
    init_generate();
    init_gameId();
...
// ../.wrangler/tmp/bundle-iWgJii/middleware-loader.entry.ts
init_functionsRoutes_0_049655092631503805();
init_checked_fetch();

// ../.wrangler/tmp/bundle-iWgJii/middleware-insertion-facade.js
init_functionsRoutes_0_049655092631503805();
init_checked_fetch();
...
//# sourceMappingURL=functionsWorker-0.16009875547304142.mjs.map
